...
class AllFeaturedServicesScreen extends ConsumerWidget {
  const AllFeaturedServicesScreen({super.key});

  @override
  Widget build(BuildContext context, WidgetRef ref) {
    // We stream all services and filter for isFeatured in the builder or query
    final servicesStream = ref.watch(firestoreServiceProvider).streamServices();

    return Scaffold(
      appBar: AppBar(title: const Text('Featured Services')),
      body: StreamBuilder<List<ServiceModel>>(
        ...
        builder: (context, snapshot) {
          ...
          final services = (snapshot.data ?? []).where((s) => s.isFeatured).toList();
          
          if (services.isEmpty) {
            return const Center(child: Text('No featured services at the moment.'));
          }
